...
const Dashboard = () => {
  ...
  const total = directReferrals + indirectReferrals;
    ...
    <>
      <div className="dashboard-container">
        ...
        <p className="dashboard-referral-link" onClick={handleCopyReferralLink} style={{ cursor: 'pointer', color: 'black' }}>
          ...
          Copy/Paste your Referral Link in your Message<br/><br/> <span style={{fontWeight: 'bold'}}> {referralLink} </span>
        </p>
          <p className="dashboard-total-registrations"  >YOUR REGISTRATIONS:  {totalRegistrations}</p>

        <div className="dashboard-referrals">
          <h3>Direct: {directReferrals}</h3>
          <h3>Viral: {indirectReferrals}</h3>
          <h3>Total: {total}</h3>
